...
domain_profiles = {
    ...
    "informatica_economica": {
        ...
        "keywords": ["business", "economic", "financial", "erp", "business_analytics", "applications", "case_study", "fintech"],
        "universities": [
            {
                "name": "ASE București",
    ...
    "automatica": {
        ...
        "keywords": ["hardware", "systems", "engineering", "embedded", "low_level", "robotics", "industrial", "iot", "technical"],
        "universities": [
            {
...
def calculate_integrated_confidence(domain_scores, kmeans_result, rf_result):
    ...
    integrated_confidence = min(1.0, base_confidence_score + kmeans_boost + rf_boost + algorithm_consensus + distance_factor + probability_factor)
    ...
    return {
        ...
        "interpretation": get_improved_confidence_interpretation(integrated_confidence_percent, base_confidence["confidence_percent"])
    }
...
def explain_score_enhancements(original_scores, enhanced_scores, kmeans_result, rf_result):
    ...
    if "cluster_domains" in kmeans_result and best_domain in kmeans_result["cluster_domains"]:
        explanation["details"].append("Analiza de clustering (KMeans) a confirmat că profilul tău se încadrează în grupul specific pentru acest domeniu.")
    
    if "predicted_domain" in rf_result and rf_result["predicted_domain"] == best_domain:
        probability = rf_result.get("probabilities", {}).get(best_domain, 0)
        probability_percent = int(probability * 100)
        explanation["details"].append(f"Modelul de clasificare (Random Forest) a prezis acest domeniu cu o probabilitate de {probability_percent}%.")
    ...
    if "feature_importances" in rf_result and len(rf_result["feature_importances"]) > 0:
        ...
        explanation["details"].append(f"Caracteristicile tale cele mai relevante pentru această predicție sunt: {', '.join(top_features)}.")
